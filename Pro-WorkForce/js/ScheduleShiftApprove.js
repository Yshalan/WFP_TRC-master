...
var xhrLeave = null; // XMLHttpRequest
var infoJSON = null; // eval(chr)
var LeaveDetailsJSON = null;
var LeaveJSON = null;
...
var FilterOption = 1; //Filter Option (1 All, 2 Pending Only, 3 Approve Only)
...
function ddlWorkSchedule_changed(selectedSchedule, FilterOption) {
    ...
    // ismail5 to remove other schedule data
    cntrl_divColor.innerHTML = '';
    SelectedShift = null;
    selectedCol.style.backgroundColor = '#FFFFFF';
    setInnerText(selectedTxt, '');
    ...
    var divScheduleDetails2 = document.createElement("div");
    divScheduleDetails2.setAttribute('class', 'divShiftClass');
    var ShiftColor2 = document.createElement("div");
    ShiftColor2.setAttribute('style', '');
    ShiftColor2.setAttribute('class', 'col');

    ShiftColor2.innerHTML = '<img src="../images/annual1.png" width=20 height=20  valign=absolute  />';
    var ShiftCode2 = document.createElement("label");
    ShiftCode2.innerHTML = '&nbsp; Annual Leave';
    divScheduleDetails2.appendChild(ShiftColor2);
    divScheduleDetails2.innerHTML += ' ';
    divScheduleDetails2.appendChild(ShiftCode2);
    divScheduleDetails2.innerHTML += ' ';
    divScheduleShiflegend.appendChild(divScheduleDetails2);

    var divScheduleDetails3 = document.createElement("div");
    divScheduleDetails3.setAttribute('class', 'divShiftClass');
    var ShiftColor3 = document.createElement("div");
    ShiftColor3.setAttribute('style', '');
    ShiftColor3.setAttribute('class', 'col');

    ShiftColor3.innerHTML = '<img src="../images/sick.png" width=20 height=20 valign=absolute />';
    var ShiftCode3 = document.createElement("label");
    ShiftCode3.innerHTML += '&nbsp; Sick Leave';
    divScheduleDetails3.appendChild(ShiftColor3);
    divScheduleDetails3.innerHTML += ' ';
    divScheduleDetails3.appendChild(ShiftCode3);
    divScheduleDetails3.innerHTML += ' ';
    divScheduleShiflegend.appendChild(divScheduleDetails3);

    var divScheduleDetails4 = document.createElement("div");
    divScheduleDetails4.setAttribute('class', 'divShiftClass');
    var ShiftColor4 = document.createElement("div");
    ShiftColor4.setAttribute('style', '');
    ShiftColor4.setAttribute('class', 'col');

    ShiftColor4.innerHTML = '<img src="../images/holiday.png" width=20 height=20 valign=absolute />';
    var ShiftCode4 = document.createElement("label");
    ShiftCode4.innerHTML += '&nbsp; Holiday';
    divScheduleDetails4.appendChild(ShiftColor4);
    divScheduleDetails4.innerHTML += ' ';
    divScheduleDetails4.appendChild(ShiftCode4);
    divScheduleDetails4.innerHTML += ' ';
    divScheduleShiflegend.appendChild(divScheduleDetails4);
...
function GetData(FilterOption) {
    //	send XMLHttpRequest to get list of employees and their shifts according to selected schedule
    var Url = "../Handlers/GetEmployeeScheduleApprove.ashx?year=" + year + "&month=" + month + "&schedule=" + SelectedSchedule_Global + "&r=" + Math.random().toString() + "&FilterOption=" + FilterOption;
...
function GetLeaveData() {
    //	send XMLHttpRequest to get list of employee leave details for the selected month

    var Url = "../Handlers/GetEmployeeLeaveDetails.ashx?year=" + year + "&month=" + month + "&r=" + Math.random().toString();
    xhrLeave = new XMLHttpRequest();
    xhrLeave.onreadystatechange = ResponseReceivedLeaveDetails;
    xhrLeave.open("GET", Url, true);
    xhrLeave.send(null);
...
function ResponseReceived() {
    ...
    if (xhr.readyState == 4 && xhr.status >= 400) {
        var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
        if (UICulture.value == "ar-JO") {
            ShowMessage_Errorlobibox('خطأ');
        } else {
            ShowMessage_Errorlobibox('Error');
        }
    }
}
function ResponseReceivedLeaveDetails() {
    var data2 = null;
    if (xhrLeave.readyState == 4 && xhrLeave.status == 200) {
        if ((xhrLeave.responseText) != '[{\"leaves\":]}]') {
            data2 = eval(xhrLeave.responseText);
            if (data2) {

                LeaveDetailsJSON = data2;
                ProcessData();
            }
        }
        else {
    ...
    if (xhrLeave.readyState == 4 && xhrLeave.status >= 400) {
        var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
        if (UICulture.value == "ar-JO") {
            ShowMessage_Errorlobibox('خطأ');
        } else {
...
function ProcessData() {
    ...
    if (LeaveDetailsJSON != null) {
        LeaveJSON = LeaveDetailsJSON[0].leaves;
        if (LeaveJSON == undefined) {
            if (!LeaveDetailsJSON[0][0].leaves == null) {
                LeaveJSON = LeaveDetailsJSON[0][0].leaves;
            }
...
function PostData(FilterOption) {
    //if (scheduleJSON) {
        var dataToPost = jsonPath(scheduleJSON, "$[?(@.status != \"4\")]");// in order to check modification and approve even if no changes were made
        if (dataToPost) {
            cntrl_imgLoading.style.display = '';
            cntrl_btnSave.disabled = 'disabled';
            cntrl_btnClear.disabled = 'disabled';

            var Url = "../Handlers/GetEmployeeScheduleApprove.ashx?year=" + year + "&month=" + month + "&schedule=" + SelectedSchedule_Global + "&FilterOption=" + FilterOption;
    ...
    if (xhr.readyState == 4) {
        if (xhr.status == 200) {
            ...
            if (AffectedRows > 0) {
                var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
                if (UICulture.value == "ar-JO") {
                    ShowMessage_Successlobibox('تم الحفظ بنجاح'); //ShowMessage('AffectedRows: ' + AffectedRows);
                } else {
                    ShowMessage_Successlobibox('Saved Successfully'); //ShowMessage('AffectedRows: ' + AffectedRows);
                }
            }
                ...
                var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
                if (UICulture.value == "ar-JO") {
                    ShowMessagelobibox(ValidationHrs); //ShowMessage('AffectedRows: ' + AffectedRows);
                } else {
                    ShowMessagelobibox(ValidationHrs); //ShowMessage('AffectedRows: ' + AffectedRows);
                }
            }
                ...
                var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
                if (UICulture.value == "ar-JO") {
                    ShowMessage_Successlobibox('تمت العملية بنجاح, ولكن بدون اي تغيير على البيانات');
                } else {
                    ShowMessage_Successlobibox('Process Completed, but no data changed');
                }
            }
...
function ShowMessagelobibox(msg) {
    Lobibox.alert('info', {
        title: '&nbsp;',
        size: 'large',
        // delay: false,
...
function ShowMessage_Successlobibox(msg) {
...
function ShowMessage_Errorlobibox(msg) {
    Lobibox.alert('error', {
        title: '&nbsp;',
        size: 'large',
        // delay: false,
...
function RemoveSchedule(year, month, day, EmpID, td) {
    if (rightButtonDown) {
        ...
        if ((new Date(year, month - 1, day).getDay() == 6) || new Date(year, month - 1, day).getDay() == 5) {
    ...
    if (leftButtonDown) {
        ...
        if (!!SelectedShift) {
            ...
            for (var i = 0; i < scheduleJSON.length; i++) {
                if (scheduleJSON[i].EmpID == EmpID && scheduleJSON[i].day == day && scheduleJSON[i].month == month && scheduleJSON[i].year == year) {
                    scheduleJSON.splice(i, 1);
                }
            }
            if (!!scheduleJSON) {
                var _currentSchedule = jsonPath(scheduleJSON, "$[?(@.year == " + year + " && @.month == " + month + " && @.day == " + day + " && @.EmpId == " + EmpID + ")]");
...
function YearMonthChanged() {
    ...
    var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
    var ArchivingMonhts = document.getElementById("ctl00_ContentPlaceHolder1_hdnArchivingMonths");
    if (ArchivingMonhts.value != 0) {
        var SelectedMonth = cntrl_ddlMonth.options[cntrl_ddlMonth.selectedIndex].value - 1;
        var SelectedYear = cntrl_ddlYear.options[cntrl_ddlYear.selectedIndex].value;
        var d = new Date();
        var SelectedDay = d.getDate();
        var SelectedDate = new Date(), y = SelectedYear, m = SelectedMonth, d = SelectedDay;
        SelectedDate = new Date(y, m, d);
        SelectedDate = formatDate(SelectedDate);
        var ArchiveDate = new Date();
        ArchiveDate.setMonth(ArchiveDate.getMonth() - ArchivingMonhts.value);
        ArchiveDate = formatDate(ArchiveDate);

        if (SelectedDate <= ArchiveDate) {
    ...
    if (conf) {
        ...
        //cntrl_ddlWorkSchedule.selectedIndex = 0; //for clear the Work Schedule after change Year and Month
        ClearScheduleGrid();
...
function ClearScheduleGrid() {
    ...
    for (var j = 1; j <= new Date(year, month, 0).getDate() ; j++) {
        ...
        if (new Date(year, month - 1, j).getDay() == 6 || new Date(year, month - 1, j).getDate() == new Date(year, month + 1, 0).getDate()) {
...
function scheduleGrid_AddEMP(EmpId) {
    ...
    if (!!CurrentGroup) {
        ...
        for (var j = 1; j <= new Date(year, month, 0).getDate() ; j++) {
            ...
            // ismail & kiran working here

            var existingLeave = jsonPath(LeaveJSON, "$[?(@.theday == " + j + " && @.EmployeeId == " + CurrentGroup[0].EmpId + ")]");
            if (!!existingLeave) {
                if (existingLeave[0].LeaveTypeId == 68) {
                    var sick = document.createElement("img");
                    sick.setAttribute("src", "../images/sick.png");
                    sick.setAttribute("width", "12");
                    sick.setAttribute("height", "12");
                    td_b.appendChild(sick);
                }
                else if (existingLeave[0].LeaveTypeId == 0) {
                    var Holiday = document.createElement("img");
                    Holiday.setAttribute("src", "../images/Holiday.png");
                    Holiday.setAttribute("width", "12");
                    Holiday.setAttribute("height", "12");
                    td_b.appendChild(Holiday);
                }
                else {
                    var annual1 = document.createElement("img");
                    annual1.setAttribute("src", "../images/annual1.png");
                    annual1.setAttribute("width", "12");
                    annual1.setAttribute("height", "12");
                    td_b.appendChild(annual1);
                }
            ...
            if (new Date(year, month - 1, j).getDay() == 6 || new Date(year, month - 1, j).getDate() == new Date(year, month + 1, 0).getDate()) {
                ...
                setInnerText(td_hsum, displayTime(workhours));
...
function btnAddEmp_Click() {
    if (Page_ClientValidate('AddEmployeeGroup') && Page_ClientValidate('WorkSchedule')) {
        ...
        if (id != null) {
            ...
            var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
            if (UICulture.value == "ar-JO") {
                ShowMessagelobibox("لم يتم العثور على الموظف رقم.: " + cntrl_txtEmpNo.value + ", الرجاء التاكد ان الموظف المختار لديه جدول عمل متقدم");
            }
            else {
                ShowMessagelobibox("Cannot find employee with no.: " + cntrl_txtEmpNo.value + ", Make sure selected employee has advanced schedule");
            }
        }
    }
}
...
JSON Classes:
1. employeesJSON
- this JSON list is created in the generic handler ~/Handlers/GetEmployeeSchedule4.ashx
