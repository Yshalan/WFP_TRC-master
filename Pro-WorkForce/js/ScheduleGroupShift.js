...
var groupJSON = null; // list of Groups
...
function ClearForm() {
    ...
    save();
    location.href = "../ScheduleGroup/PrepareShiftSchedule.aspx";
...
function GetData() {
    ...
    var Url = "../Handlers/GetGroupSchedule.ashx?year=" + year + "&month=" + month + "&schedule=" + SelectedSchedule_Global + "&r=" + Math.random().toString();
...
function ProcessData() {
    // convert received response to JSON data
      ...
      groupJSON = infoJSON[0].Groups;
   
    if (groupJSON == undefined) {
       
        groupJSON = infoJSON[0][0].Groups;
    }
  
    //FillAutoCompleteList();
  
    scheduleGrid_AddAllGroup(hdnGroupIDs.value);
...
function PostData() {
    if (scheduleJSON) {
        ...
        if (dataToPost) {
            ...
            var Url = "../Handlers/GetGroupSchedule.ashx?year=" + year + "&month=" + month + "&schedule=" + SelectedSchedule_Global;
    ...
    if (xhr.readyState == 4) {
        if (xhr.status == 200) {
            ...
            if (data) {
                ...
                if (!!lst) {
                    ...
                    for (i; i < lst.length; i++) {
                        var CurrentEmp = jsonPath(groupJSON, "$[?(@.GroupID == " + lst[i].GroupID + ")]");
            ...
            if (AffectedRows > 0) {
                var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
                if (UICulture.value == "ar-JO") {
                    alert('تم الحفظ بنجاح'); //ShowMessage('AffectedRows: ' + AffectedRows);
                } else {
                    alert('Saved Successfully'); //ShowMessage('AffectedRows: ' + AffectedRows);
                }
            }
            else {
                var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
                if (UICulture.value == "ar-JO") {
                    alert('تمت العملية بنجاح, ولكن بدون اي تغيير على البيانات');
                } else {
                    alert('Process Completed, but no data changed');
                }
            }
...
function FillAutoCompleteList() {
    ...
    var lst = jsonPath(groupJSON, "$[?(@.listed == \"false\")]");
    
    if (!!lst)
        GroupAutocompleteSrc = jQuery.map(lst, function (e) { return { label: e.GroupName, value: e.GroupID }; }); //empAutocompleteSrc = lst.map(function(e) { return { label: e.EmpName, value: e.EmpId }; });
    else
...
function CreateScheduleNode(year, month, day, GroupID) {
    return eval('[{ "GroupID": "' + GroupID
...
function RemoveSchedule(year, month, day, GroupID, td) {
    if (rightButtonDown) {
        ...
        var _currentSchedule = jsonPath(scheduleJSON, "$[?(@.year == " + year + " && @.month == " + month + " && @.day == " + day + " && @.GroupID == " + GroupID + ")]");
        if (!!_currentSchedule) {
...
function ChangeShift(year, month, day, GroupID, td, e) {
    ...
    if (leftButtonDown) {
        if (!!SelectedShift) {
            ...
            if (!!scheduleJSON) {
                ...
                if (!!_currentSchedule) {
                    ...
                    currentSchedule = CreateScheduleNode(year, month, day, GroupID)[0];
                ...
                currentSchedule = CreateScheduleNode(year, month, day, GroupID);
                currentSchedule.status = __ADD;
        ...
        return RemoveSchedule(year, month, day, GroupID, td);
    }
}
...
function td_delete_click(GroupID, tr) {
    ...
    if (conf) {
        scheduleGrid_RemoveEmp(GroupID, tr);
        FillAutoCompleteList();
    }
}

function scheduleGrid_RemoveEmp(GroupID, tr) {

    var CurrentEmp = jsonPath(groupJSON, "$[?(@.GroupID == " + GroupID + ")]");
    if (!!CurrentEmp) {
        ...
        var GrpScheduleList = jsonPath(scheduleJSON, "$[?(@.GroupID == " + GroupID + ")]");
        ...
        GrpScheduleList = jsonPath(originalScheduleJSON, "$[?(@.GroupID == " + GroupID + ")]");
...
function scheduleGrid_AddAllGroup(GroupIDz) {
    var GroupID=0;
  
    if (GroupIDz && GroupIDz != '') 
    {
        var GrpIDz = GroupIDz;
        var arrGrpIDz = new Array();
        arrGrpIDz = GrpIDz.split(',');

    }
   
    if (validateDDLs() && groupJSON) {
      
        var lst = jsonPath(groupJSON, "$[?(@.listed == \"false\")]");

     
            if (arrGrpIDz && arrGrpIDz.length > 0) {

                for (var item = 0; item < arrGrpIDz.length; item++) {

                    GroupID = arrGrpIDz[item];

                    //if (arrGrpIDz[item] != '' && arrGrpIDz[item] == lst[i].GroupID)
                    if (GroupID>0) {

                        scheduleGrid_AddGroup(GroupID);
            ...
            function scheduleGrid_AddGroup(GrpId) 
 {
        ...
        var CurrentGroup = jsonPath(groupJSON, "$[?(@.GroupID == " + GrpId + ")]");
        if (!!CurrentGroup) {
            ...
            setInnerText(td_b1, CurrentGroup[0].GroupCode);
            tr_b.appendChild(td_b1);

            var td_b2 = document.createElement("td");
            setInnerText(td_b2, CurrentGroup[0].GroupName);
            ...
            for (var j = 1; j <= new Date(year, month, 0).getDate(); j++) {
                ...
                td_b.setAttribute('onmousedown', 'ChangeShift(' + year + ',' + month + ',' + j + ',' + CurrentGroup[0].GroupID + ',this,event);');
                ...
                /* BEGIN */
                td_b.setAttribute('onmouseover', 'td_b_mouseover(' + year + ',' + month + ',' + j + ',' + CurrentGroup[0].GroupID + ',this,event);');
                /* END */
                ...
                if (!!scheduleJSON) {
                    ...
                    var existing = jsonPath(scheduleJSON, "$[?(@.year == " + year + " && @.month == " + month + " && @.day == " + j + " && @.GroupID == " + CurrentGroup[0].GroupID + ")]");
            ...
            var td_d = document.createElement("td");
            td_d.setAttribute('onclick', 'td_delete_click(' + CurrentGroup[0].GroupID + ',this.parentNode)');
...
function btnAddEmp_Click() {
    if (Page_ClientValidate('AddEmployeeGroup') && Page_ClientValidate('WorkSchedule')) {
        ...
        if (id != null) {
            ...
            var UICulture = document.getElementById("ctl00_ContentPlaceHolder1_hdnUICulture");
            if (UICulture.value == "ar-JO") {
                alert("لم يتم العثور على الموظف رقم.: " + cntrl_txtEmpNo.value + ", الرجاء التاكد ان الموظف المختار لديه جدول عمل متقدم");
            }
            else {
                alert("Cannot find employee with no.: " + cntrl_txtEmpNo.value + ", Make sure selected employee has advanced schedule");
            }
        }
    }
}
...
JSON Classes:
1. employeesJSON
- this JSON list is created in the generic handler ~/Handlers/GetGroupSchedule.ashx
