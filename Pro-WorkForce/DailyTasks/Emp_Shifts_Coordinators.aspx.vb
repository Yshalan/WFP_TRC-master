...
Partial Class Admin_Emp_Shifts_Coordinators
...
#Region "Properties"
    ...
    Protected Property IsDateExists As Boolean
        Get
            Return ViewState("IsDateExists")
        End Get
        Set(ByVal value As Boolean)
            ViewState("IsDateExists") = value
        End Set
    End Property
...
#Region "Methods"
    ...
    Private Sub ShowHideSaveButton()
        ...
        If Not dt Is Nothing Then
            If dt.Rows.Count > 0 Then
                IsDateExists = dt.Rows(0)("DateExists")
            End If
        End If

    End Sub

    Private Sub FillMonthsAccordingDates()
        Dim dt As DataTable
        ...
        With objEmp_Shifts_ModificationDate
            ...
            If Not dt Is Nothing Then
                If dt.Rows.Count > 0 Then
                    DateOption = dt.Rows(0)("DateOption")
                    If DateOption = 2 Then
                        ddlMonth_BindByDates(Month(dt.Rows(0)("FromDate")), Month(dt.Rows(0)("ToDate")), Convert.ToInt32(dt.Rows(0)("DateOption")))
                        ddlYear_BindByDates(Year(dt.Rows(0)("FromDate")), Year(dt.Rows(0)("ToDate")), Convert.ToInt32(dt.Rows(0)("DateOption")))

                    Else
                        ddlMonth_BindByDates(dt.Rows(0)("FromDay"), dt.Rows(0)("ToDay"), Convert.ToInt32(dt.Rows(0)("DateOption")))
                        ddlYear_BindByDates(dt.Rows(0)("FromDay"), dt.Rows(0)("ToDay"), Convert.ToInt32(dt.Rows(0)("DateOption")))
                    End If
                End If
            End If
        End With
    ...
    Private Sub ddlMonth_BindByDates(ByVal intMonthFromDate As Integer, ByVal intMonthToDate As Integer, ByVal DateOption As Integer)
        Dim dt As New DataTable
        dt.Columns.Add("val")
        dt.Columns.Add("txt")

        For i As Integer = IIf(DateOption = 2, intMonthFromDate, Month(Date.Today)) To IIf(DateOption = 2, intMonthToDate, Month(Date.Today) + 1)
            Dim dr As DataRow = dt.NewRow()
            dr("val") = i
    ...
    Private Sub ddlYear_BindByDates(ByVal intYearFromDate As Integer, ByVal intYearToDate As Integer, ByVal DateOption As Integer)
        'Dim year As Integer = DateTime.Now.Year
        Dim dt As New DataTable
        dt.Columns.Add("val")
        dt.Columns.Add("txt")
        ...
        For i As Integer = IIf(DateOption = 2, intYearFromDate, Year(Date.Today)) To IIf(DateOption = 2, intYearToDate, Year(Date.Today) + 1)
            Dim dr As DataRow = dt.NewRow()
            dr("val") = i
            dr("txt") = i
            dt.Rows.Add(dr)
