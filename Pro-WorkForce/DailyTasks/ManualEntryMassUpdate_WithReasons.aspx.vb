...
Partial Class DailyTasks_ManualEntryMassUpdate_WithReasons
...
#Region "Page Events"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        If Not Page.IsPostBack Then
            ...
            PageHeader1.HeaderText = ResourceManager.GetString("StudentManualEntryMassUpdate", CultureInfo)
        ...
        If Not ddlReason.SelectedValue = -1 Then
            ...
            For Each item As ListItem In cblEmpList.Items
                If item.Selected Then
                    ...
                    Dim dtActiveSchedule = objWorkSchedule.GetEmployeeActiveScheduleByEmpId(RadDatePicker1.SelectedDate)
                    Dim ScheduleTimes As DataTable
                    Dim strScheduleFromTime As String = String.Empty
                    Dim strScheduleToTime As String = String.Empty
                    ...
                    If (dtActiveSchedule IsNot Nothing And dtActiveSchedule.Rows.Count > 0) Then
                        If dtActiveSchedule.Rows(0)("ScheduleType") = 1 Then
                            ...
                            ScheduleTimes = objWorkSchedule_NormalTime.GetAll()
                            strScheduleFromTime = ((ScheduleTimes.Rows(0)("FromTime1").ToString).Substring(0, 2) * 60) + ((ScheduleTimes.Rows(0)("FromTime1").ToString).Substring(3, 2))
                            strScheduleToTime = ((ScheduleTimes.Rows(0)("ToTime1").ToString).Substring(0, 2) * 60) + ((ScheduleTimes.Rows(0)("ToTime1").ToString).Substring(3, 2))
                            'CtlCommon.ConvertintToTime(ScheduleTimes.Rows(0)("FromTime1"))
                        ...
                        ElseIf dtActiveSchedule.Rows(0)("ScheduleType") = 2 Then
                            ...
                            ScheduleTimes = objWorkSchedule_Flexible.GetAll()
                            strScheduleFromTime = ((ScheduleTimes.Rows(0)("FromTime1").ToString).Substring(0, 2) * 60) + ((ScheduleTimes.Rows(0)("FromTime1").ToString).Substring(3, 2))
                            strScheduleToTime = ((ScheduleTimes.Rows(0)("ToTime1").ToString).Substring(0, 2) * 60) + ((ScheduleTimes.Rows(0)("ToTime1").ToString).Substring(3, 2))
                        ...
                        ElseIf dtActiveSchedule.Rows(0)("ScheduleType") = 3 Then
                            ...
                            objEmp_Shifts.WorkDate = RadDatePicker1.SelectedDate
                            ScheduleTimes = objEmp_Shifts.GetShiftsByDate()
                            strScheduleFromTime = ((ScheduleTimes.Rows(0)("FromTime1").ToString).Substring(0, 2) * 60) + ((ScheduleTimes.Rows(0)("FromTime1").ToString).Substring(3, 2))
                            strScheduleToTime = ((ScheduleTimes.Rows(0)("ToTime1").ToString).Substring(0, 2) * 60) + ((ScheduleTimes.Rows(0)("ToTime1").ToString).Substring(3, 2))
                        End If
                    End If

                    temp_time = CtlCommon.ConvertintToTime(strScheduleFromTime)
                    ...
                    err = objEmpMove.Add()
                    If err = -2 Then
                        CtlCommon.ShowMessage(Page, item.Text + " " + ResourceManager.GetString("TransactionAlreadyExists", CultureInfo), "info")
                        Exit Sub
                    End If
                    ...
                    If err = 0 Then
                        ...
                        temp_time = CtlCommon.ConvertintToTime(strScheduleToTime)
                        objEmpMove.MoveTime = temp_time
...
#Region "Methods"
    ...
    Private Sub FillReasons()
        ...
        CtlCommon.FillTelerikDropDownList(ddlReason, objTA_Reason.GetIsScheduleTiming, Lang)
    End Sub
    ...
    Public Sub FillGrid()
        ...
        'Next
        objEmpMove = New Emp_Move
        objEmpMove.Id = SessionVariables.LoginUser.ID
        objEmpMove.MoveDate = RadDatePicker1.SelectedDate

        dtCurrent = objEmpMove.GetFilter_ByUser()
        Dim dv As New DataView(dtCurrent)
        dv.Sort = SortExepression
        dgEmpAtt.DataSource = dv
        dgEmpAtt.DataBind()
    ...
    Private Sub GetAttend_Absent()
        ...
        With objEmpMove
            ...
            .FK_CompanyId = MultiEmployeeFilterUC.CompanyID
            temp_date = RadDatePicker1.SelectedDate
            temp_str_date = DateToString(temp_date)
            .M_DATE_NUM = temp_str_date
            .FK_EntityId = MultiEmployeeFilterUC.EntityID
            dt = .Get_Attend_Absent()

        End With

        If Not dt Is Nothing Then
            If dt.Rows.Count > 0 Then
                lblTotalEmpVal.Text = dt.Rows(0)("TotalEmployees")
                lblAttendEmpVal.Text = dt.Rows(0)("AttendEmployees")
                lblAbsentEmpVal.Text = dt.Rows(0)("AbsentEmployees")
            Else
                lblTotalEmpVal.Text = "0"
                lblAttendEmpVal.Text = "0"
                lblAbsentEmpVal.Text = "0"
            End If
        Else
            lblTotalEmpVal.Text = "0"
            lblAttendEmpVal.Text = "0"
