...
Partial Class Requests_UserControls_HR_HR_PermissionApproval
...
#Region "Page Events"
    ...
    Protected Sub Page_Load(ByVal sender As Object, ByVal e As System.EventArgs) Handles Me.Load
        ...
        If Not IsPostBack Then
            ...
            lblPermissionRequests.Text = ResourceManager.GetString("HRPermitApprove", CultureInfo)
            Page.Title = "Work Force Pro : :" + ResourceManager.GetString("HRPermitApprove", CultureInfo)
            ShowHideControls()
            FillGridView()
        End If
    End Sub
    ...
    Protected Sub lnkAccept_Click(ByVal sender As Object, ByVal e As System.EventArgs)
        ...
        With objAPP_Settings
            ...
            If (PermissionApproval = 2 Or PermissionApproval = 3) Then
                ...
                Dim days As String = DirectCast(DirectCast(sender, LinkButton).Parent.Parent, GridDataItem).GetDataKeyValue("Days")
                ...
                PermissionId = 0
                If IsFlexible Then
                    If (objEmp_PermissionsRequest.ValidateEmployeeFlexiblePermission(SessionVariables.LoginUser.FK_EmployeeId, PermissionId, dtCurrent, IsForPeriod, _
                                                                   PermDate, ToDate, PermDate, objPermissionsTypes, PermTypeId, _
                    ...
                    Else
                        objEmp_Permissions.AddPermAllProcess(EmployeeId, PermTypeId, False, IsFlexible, False, Remarks, FromDate, _
                                                             ToDate, Not IsForPeriod, FromTime, ToTime, PermDate, PermissionDaysCount, OffAndHolidayDays, _
                ...
                Else
                    ...
                    Else
                        objEmp_Permissions.AddPermAllProcess(EmployeeId, PermTypeId, False, IsFlexible, False, Remarks, FromDate, _
                                                             ToDate, Not IsForPeriod, FromTime, ToTime, PermDate, PermissionDaysCount, OffAndHolidayDays, _
                                                             days, EmpLeaveTotalBalance, PermissionId, 1, 0, AttachedFile, IsFullyDay, Nothing, ErrorMessage)
                    End If
                End If
                ...
                With objEmp_PermissionsRequest
                    ...
                    End If
                End With
            ElseIf PermissionApproval = 4 Then
                With objEmp_PermissionsRequest
                    .PermissionId = CStr(DirectCast(DirectCast(sender, LinkButton).Parent.Parent, GridDataItem).GetDataKeyValue("PermissionId"))
                    .GetByPK()
                    .FK_StatusId = RequestStatus.ApprovedByHR
                    .LAST_UPDATE_BY = SessionVariables.LoginUser.ID
                    .FK_HREmployeeId = SessionVariables.LoginUser.FK_EmployeeId
    ...
    Protected Sub dgrdEmpLeaveRequest_NeedDataSource(ByVal sender As Object, ByVal e As Telerik.Web.UI.GridNeedDataSourceEventArgs) Handles dgrdEmpLeaveRequest.NeedDataSource
        ...
        dtCurrent = objEmp_PermissionsRequest.GetByHR()
...
#Region "Methods"
    ...
    Public Sub FillGridView()
        Try
            ...
            If Not objAPP_Settings.PermApprovalFromPermission = True Then
                PermissionApproval = objAPP_Settings.PermApprovalFromPermission
                objEmp_PermissionsRequest.FK_StatusId = PermissionApproval
            End If

            'dgrdEmpLeaveRequest.DataSource = objEmp_PermissionsRequest.GetByDirectManager()
            dtPermissionRequet = objEmp_PermissionsRequest.GetByHR()
            dtCurrent = dtPermissionRequet
    ...
    Private Sub AcceptAll(ByVal item As GridDataItem, ByRef ErrNo As Integer, ByRef Msg As String)
        ...
        With objAPP_Settings
            ...
            If (PermissionApproval = 2 Or PermissionApproval = 3) Then
                ...
                Dim days As String = item.GetDataKeyValue("Days")
            ...
            ElseIf PermissionApproval = 4 Then
                With objEmp_PermissionsRequest
                    .PermissionId = CStr(item.GetDataKeyValue("PermissionId"))
                    .GetByPK()
                    .FK_StatusId = RequestStatus.ApprovedByHR
                    .LAST_UPDATE_BY = SessionVariables.LoginUser.ID
                    .FK_HREmployeeId = SessionVariables.LoginUser.FK_EmployeeId
