...
Public Class GetEmployeeSchedule2 : Implements IHttpHandler, IRequiresSessionState
    ...
    Private Sub SaveScheduleXML(ByVal context As HttpContext)
        ...
        errNo = objScheduleJSON.InsertUpdateDeleteScheduleRequest(lstScheduleJSON, AffectedRows)
        ...
        If errNo = 0 Then
            errNo = objScheduleJSON.InsertUpdateDeleteScheduleApprove(lstScheduleJSON, AffectedRows)
            If errNo = -999 Then
                Dim dt As DataTable
                Dim EmployeeNo As String = ""
                Dim EmployeeName As String = ""
        ...
        JSONData = ",[{""AffectedRows"":" + "1" + ", " + "}],[{""ValidationHrs"":" + """" + ValidationHrs.ToString() + """" + ", " + "}]"
    ...
    Private Function gets(ByVal lstScheduleJSON As List(Of ScheduleJSON)) As DataTable
        ...
        For Each json As ScheduleJSON In lstScheduleJSON
            ...
            CreatedbyElement.InnerText = json.CreatedBy
            scheduleElement.AppendChild(CreatedbyElement)
            
        Next
        ScheduleXml.Document = doc
        dt = objEmp_Shifts.GetRequestValidtionMsg(ScheduleXml)
        Return dt
    End Function
    ...
    Private Sub GetScheduleXML(ByVal context As HttpContext)
        ...
        FilterOption = Convert.ToInt32(context.Request.QueryString("FilterOption"))
        ...
        Dim ManagerId As Integer = SessionVariables.LoginUser.FK_EmployeeId
        Dim CreatedBy As String = SessionVariables.LoginUser.ID
        Dim FK_StatusId As Integer = 2 '--Approve By Manager
        JSON = "[{""employees"":" + objEmp_WorkSchedule.GetAllEmployees_JSON2(Month, Year, scheduleId, ManagerId, FilterOption) + ", " + """schedule"":" + objEmp_Shifts.GetRequestByDate_JSON(Year, Month, FK_StatusId, CreatedBy) + "}]"
