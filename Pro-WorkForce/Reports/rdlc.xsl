...
>
  ...
  <xsl:variable name="mvarLeft" select="/xs:schema/@Left"/>
  <xsl:variable name="mvarLeft2" select="/xs:schema/@Left2"/>
  <xsl:variable name="mvarTitle" select="/xs:schema/@Title"/>
  <xsl:variable name="mvarFontSize">10pt</xsl:variable>
      ...
      <Body>
        ...
        <ReportItems>
          <Textbox Name="txtitle">
            ...
            <Left>
              <xsl:value-of select="$mvarLeft2" />
            </Left>
            <Height>0.63492cm</Height>
            <Value>
              <xsl:value-of select="$mvarTitle" />
            </Value>
          </Textbox>
    ...
    <TableCell>
      <ReportItems>
        <Textbox Name="textbox{position()}">
          ...
          <Value>
            <xsl:value-of select="translate($varFieldName, '_',' ')"/>
          </Value>
          <CanGrow>true</CanGrow>
          <ZIndex>7</ZIndex>
          <Style>
  ...
  <xsl:param name="repstr1">_</xsl:param>
  <xsl:param name="repstr2">
    <xsl:text> </xsl:text>
  </xsl:param>
  ...
  <xsl:template name="replace-string">
    ...
    <xsl:choose>
      ...
      <xsl:when test="contains($str, $repstr1)">
        <xsl:value-of select="substring-before($str, $repstr1)"/>
        <xsl:value-of select="$repstr2"/>
        <xsl:call-template name="replace-string">
          <xsl:with-param name="str" select="substring-after($str, $repstr1)"/>
        </xsl:call-template>
      </xsl:when>
